...
@Entity
@Table(name = "SB_CUSTOMER_ORDER_07")
public class SBCustomerOrder07
{
    final static Logger LOGGER = LogManager.getLogger(SBCustomerOrder07.class);

    @Id
    @Column(name = "customer_order_07_id", columnDefinition = "BINARY(16)")
    ...
    private byte[] customerOrder07Id;

    @ManyToOne
    @JoinColumn(name = "customer_11_id")
    private SBCustomer11 sbCustomer11;

    @ManyToMany
    private Collection<SBItem10> customerOrder07Items = new ArrayList();

    @Column(name = "customer_order_07_invoice_number")
    private String customerOrder07InvoiceNumber;

    @Column (name = "customer_order_07_date_time")
    private Timestamp customerOrder07DateTime;

    @Column (name = "customer_order_07_total")
    private Double customerOrder07Total;
